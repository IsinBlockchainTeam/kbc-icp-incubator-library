...
#gcloud compute instances start coffee-trading-instance
gcloud compute ssh ci-cd-pipeline@coffee-trading-instance --quiet --command="mkdir -p ~/$PROJECT_FOLDER"
gcloud compute scp .env gcloud_vm_deploy.sh docker-compose-prod.yml ci-cd-pipeline@coffee-trading-instance:~/$PROJECT_FOLDER --quiet
gcloud compute ssh ci-cd-pipeline@coffee-trading-instance --quiet \
