...
const env = getRequiredEnvs('COMMISSIONER_PRIVATE_KEY', 'RPC_URL', 'TOKEN_ADDRESS', 'DOWN_PAYMENT_ADDRESS');
...
const main = async (amount: number) => {
    ...
    const tx = await myToken.connect(wallet).approve(env.DOWN_PAYMENT_ADDRESS, amount);
    const receipt = await tx.wait();
    console.log('receipt:', receipt);
    console.log('Allowed:', await myToken.allowance(wallet.address, env.DOWN_PAYMENT_ADDRESS));
    console.log('Balance', await myToken.connect(wallet).balanceOf(wallet.address));
};

main(100).catch(console.error); // Down payment address
