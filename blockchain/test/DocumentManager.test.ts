...
describe('DocumentManager', () => {
    ...
    const transactionLineIds = [BigNumber.from(4), BigNumber.from(5)];
    ...
    describe('registerDocument', () => {
        it('should register a document (as trade manager contract)', async () => {
            await documentManagerContract.connect(tradeManager).registerDocument(transactionId, transactionTypes[0], rawDocument.name, rawDocument.documentType, rawDocument.externalUrl, transactionLineIds);
            ...
            expect(savedDocument.transactionLineIds).to.deep.equal(transactionLineIds);
        });

        it('should register a document - FAIL (Sender has no permissions)', async () => {
            await expect(documentManagerContract.connect(sender).registerDocument(transactionId, transactionTypes[0], rawDocument.name, rawDocument.documentType, rawDocument.externalUrl, transactionLineIds))
        ...
        it('should register a document - FAIL (The transaction type specified isn\'t registered)', async () => {
            await expect(documentManagerContract.connect(owner).registerDocument(transactionId, 'custom type', rawDocument.name, rawDocument.documentType, rawDocument.externalUrl, transactionLineIds))
        ...
        it('should register a document - FAIL (The document type isn\'t registered)', async () => {
            await expect(documentManagerContract.connect(owner).registerDocument(transactionId, transactionTypes[0], rawDocument.name, 'custom document type', rawDocument.externalUrl, transactionLineIds))
                .to.be.revertedWith('The document type isn\'t registered');
        });

        it('should emit DocumentRegistered event', async () => {
            documentCounterId = await documentManagerContract.connect(sender).getDocumentsCounterByTransactionIdAndType(transactionId, transactionTypes[0]);
            await expect(documentManagerContract.connect(owner).registerDocument(transactionId, transactionTypes[0], rawDocument.name, rawDocument.documentType, rawDocument.externalUrl, transactionLineIds))
    ...
    describe('getTransactionDocumentIds', () => {
        it('should retrieve saved document ids by owner address and relative transaction id', async () => {
            await documentManagerContract.connect(owner).registerDocument(transactionId, transactionTypes[1], rawDocument.name, rawDocument.documentType, rawDocument.externalUrl, transactionLineIds);
            await documentManagerContract.connect(owner).registerDocument(transactionId, transactionTypes[1], rawDocument2.name, rawDocument2.documentType, rawDocument2.externalUrl, transactionLineIds);
