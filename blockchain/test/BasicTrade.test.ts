...
describe('BasicTrade.sol', () => {
    ...
    describe('Lines', () => {
        it('should add a line, retrieve it and update it', async () => {
            ...
            const lineId = receipt.events.find((event: Event) => event.event === 'TradeLineAdded')
                .args[0];
            expect(lineId).to.equal(1);

            const [id, productCategoryId, quantity, unit, materialId, exists] =
            ...
            expect(
                updatedReceipt.events.find((event: Event) => event.event === 'TradeLineUpdated')
                    .args[0]
            ).to.equal(lineId);

            const [
                updatedId,
                updatedProductCategoryId,
                updatedQuantity,
                updatedUnit,
                updatedMaterialId,
                updatedExists
            ...
            const secondLineId = secondReceipt.events.find(
                (event: Event) => event.event === 'TradeLineAdded'
            ).args[0];
        ...
        it('should assign a material to a line', async () => {
            ...
            const [id, productCategoryId, quantity, unit, materialId, exists] =
                await basicTradeContract.getLine(1);
            ...
            expect(exists).to.equal(true);
            expect(
                receipt.events.find((event: Event) => event.event === 'MaterialAssigned').args[0]
            ).to.equal(1);
        });

        it('should add a line - FAIL (Trade: Product category does not exist)', async () => {
            await expect(basicTradeContract.addLine(20, 60, units[0])).to.be.revertedWith(
                'Trade: Product category does not exist'
            );
        });

        it('should add a line - FAIL (Trade: Unit has not been registered)', async () => {
            await expect(basicTradeContract.addLine(1, 60, 'custom unit')).to.be.revertedWith(
                'Trade: Unit has not been registered'
            );
        });

        it('should get a line - FAIL (Trade: Line does not exist)', async () => {
            await expect(basicTradeContract.getLine(0)).to.be.revertedWith(
                'Trade: Line does not exist'
            );
        });

        it('should update a line - FAIL (Trade: Line does not exist)', async () => {
            await expect(basicTradeContract.updateLine(0, 5, 50, units[1])).to.be.revertedWith(
                'Trade: Line does not exist'
            );
        });
        ...
        it('should update a line - FAIL (Trade: Product category does not exist)', async () => {
            ...
            const lineId = receipt.events.find((event: Event) => event.event === 'TradeLineAdded')
                .args[0];
            await expect(
                basicTradeContract.updateLine(lineId, 20, 70, units[1])
            ).to.be.revertedWith('Trade: Product category does not exist');
        });
        ...
        it('should update a line - FAIL (Trade: Unit has not been registered)', async () => {
            ...
            const lineId = receipt.events.find((event: Event) => event.event === 'TradeLineAdded')
                .args[0];
            await expect(
                basicTradeContract.updateLine(lineId, 1, 70, 'custom unit')
            ).to.be.revertedWith('Trade: Unit has not been registered');
        });

        it('should assign a material to a line - FAIL (Trade: Line does not exist)', async () => {
            await expect(basicTradeContract.assignMaterial(0, 1)).to.be.revertedWith(
                'Trade: Line does not exist'
            );
        });
        ...
        it('should assign a material to a line - FAIL (Trade: Material does not exist)', async () => {
            ...
            await tx.wait();
            await expect(basicTradeContract.assignMaterial(1, 20)).to.be.revertedWith(
                'Trade: Material does not exist'
            );
        });
    });
