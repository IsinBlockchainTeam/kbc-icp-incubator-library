...
describe('AssetOperationManager', () => {
    ...
    beforeEach(async () => {
        ...
        processTypeManagerContractFake.contains.returns((value: string) =>
            processTypes.includes(value[0])
        );

        const AssetOperationManager = await ethers.getContractFactory(
    ...
    describe('Register', () => {
        it('should register an AssetOperation', async () => {
            const previousAssetOperationCounter =
                await assertOperationManagerContract.getAssetOperationsCounter();
            ...
            const currentAssetOperationCounter =
                await assertOperationManagerContract.getAssetOperationsCounter();
            expect(currentAssetOperationCounter).to.be.equal(1);

            const registeredTransformation =
                await assertOperationManagerContract.getAssetOperation(1);
            ...
            expect(await assertOperationManagerContract.getAssetOperationExists(1)).to.be.equal(
                true
            );
            await expect(tx)
                .to.emit(assertOperationManagerContract, 'AssetOperationRegistered')
                .withArgs(
                    registeredTransformation[0],
                    registeredTransformation[1],
                    registeredTransformation[3]
                );
            expect(await assertOperationManagerContract.getAssetOperationType(1)).to.be.equal(1);

            expect(
                await assertOperationManagerContract.getAssetOperationIdsOfCreator(admin.address)
            ).deep.equal([BigNumber.from(1)]);
            expect(
                await assertOperationManagerContract.getAssetOperationIdsOfCreator(other.address)
            ).deep.equal([]);
        });
        ...
        it('should register a Consolidation', async () => {
            ...
            await expect(tx)
                .to.emit(assertOperationManagerContract, 'AssetOperationRegistered')
                .withArgs(
                    registeredConsolidation[0],
                    registeredConsolidation[1],
                    registeredConsolidation[3]
                );
        ...
        it('should register an AssetOperation - FAIL(AssetOperationManager: At least one process type must be specified)', async () => {
            await expect(
                ...
                'AssetOperationManager: At least one process type must be specified'
            );
        });
        ...
        it('should get AssetOperation type - FAIL(AssetOperationManager: Asset operation does not exist)', async () => {
            await expect(
                assertOperationManagerContract.getAssetOperationType(1)
            ).to.be.revertedWith('AssetOperationManager: Asset operation does not exist');
        });
    });

    describe('Update', () => {
        it('should update an AssetOperation', async () => {
            await assertOperationManagerContract.registerAssetOperation(
                'testTransformation',
                [[1], [2]],
                3,
                '38.8951',
            ...
            expect(tx)
                .to.emit(assertOperationManagerContract, 'AssetOperationUpdated')
                .withArgs(updatedTransformation[0]);
        });
    });
    ...
    describe('Roles', () => {
        it('should add and remove admin roles', async () => {
            ...
            expect(
                await assertOperationManagerContract.hasRole(
                    await assertOperationManagerContract.ADMIN_ROLE(),
                    other.address
                )
            ).to.equal(true);
        ...
        it('should fail to add and remove admin roles if the caller is not an admin', async () => {
            await expect(
                assertOperationManagerContract.connect(other).addAdmin(other.address)
            ).to.be.revertedWith('AssetOperationManager: Caller is not the admin');
        });
    });
});
