...
describe('EscrowManager.sol', () => {
    ...
    async function registerNewEscrow(payee: SignerWithAddress, duration: number): Promise<number> {
        const tx = await escrowManagerContract.registerEscrow(0, payee.address, duration, tokenContract.address);
    ...
    describe('Update', () => {
        it('should update fee recipient address', async () => {
            ...
            expect(await escrowManagerContract.getFeeRecipient()).to.equal(other.address);
    ...
    describe('Escrow by shipment', () => {
        it('should get escrow by shipment', async () => {
            expect(await escrowManagerContract.getEscrowByShipmentId(0)).to.equal(ethers.constants.AddressZero);
            await registerNewEscrow(payee, duration);
            expect(await escrowManagerContract.getEscrowByShipmentId(0)).to.not.equal(ethers.constants.AddressZero);
        });
    });
    describe('Escrow creation', () => {
        ...
        it('should fail escrow registration if payee is zero address', async () => {
            await expect(escrowManagerContract.registerEscrow(0, ethers.constants.AddressZero, duration, tokenContract.address)).to.be.revertedWith(
                'EscrowManager: payee is the zero address'
            );
        });
        it('should fail escrow registration if duration is zero', async () => {
            await expect(escrowManagerContract.registerEscrow(0, payee.address, 0, tokenContract.address)).to.be.revertedWith(
                'EscrowManager: duration is zero'
            );
        });
        it('should fail escrow registration if token address is zero address', async () => {
            await expect(escrowManagerContract.registerEscrow(0, payee.address, duration, ethers.constants.AddressZero)).to.be.revertedWith(
