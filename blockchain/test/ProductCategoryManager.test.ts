...
describe('ProductCategoryManager', () => {
    ...
    const signedProof: KBCAccessControl.RoleProofStruct = {
        signedProof: '0x',
        delegator: ''
    };
    ...
    beforeEach(async () => {
        ...
        signedProof.delegator = admin.address;
    ...
    describe('Registration', () => {
        it('should register two ProductCategories', async () => {
            const previousProductCategoryCounter = await productCategoryManagerContract.getProductCategoriesCounter(signedProof);
            expect(previousProductCategoryCounter).to.be.equal(0);
            const tx = await productCategoryManagerContract.registerProductCategory(signedProof, 'testProductCategory', 85, 'firstProductCategory');
            await tx.wait();

            let currentProductCategoryCounter = await productCategoryManagerContract.getProductCategoriesCounter(signedProof);
            expect(currentProductCategoryCounter).to.be.equal(1);

            const registeredProductCategory = await productCategoryManagerContract.getProductCategory(signedProof, 1);
            ...
            expect(await productCategoryManagerContract.getProductCategoryExists(signedProof, 1)).to.be.equal(true);
            expect(tx)
            ...
            await tx2.wait();
            currentProductCategoryCounter = await productCategoryManagerContract.getProductCategoriesCounter(signedProof);
            expect(currentProductCategoryCounter).to.be.equal(2);
            const registeredProductCategory2 = await productCategoryManagerContract.getProductCategory(signedProof, 2);
    ...
    describe('Update', () => {
        it('should update a material', async () => {
            await productCategoryManagerContract.registerProductCategory(signedProof, 'testProductCategory', 85, 'firstProductCategory');
