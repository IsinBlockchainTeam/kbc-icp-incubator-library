...
describe('ProductCategoryManager', () => {
    ...
    describe('Registration', () => {
        it('should register two ProductCategories', async () => {
            const previousProductCategoryCounter =
                await productCategoryManagerContract.getProductCategoriesCounter();
            expect(previousProductCategoryCounter).to.be.equal(0);
            const tx = await productCategoryManagerContract.registerProductCategory(
                'testProductCategory',
                85,
            ...
            let currentProductCategoryCounter =
                await productCategoryManagerContract.getProductCategoriesCounter();
            expect(currentProductCategoryCounter).to.be.equal(1);

            const registeredProductCategory =
                await productCategoryManagerContract.getProductCategory(1);
            ...
            expect(await productCategoryManagerContract.getProductCategoryExists(1)).to.be.equal(
                true
            );
            expect(tx)
                .to.emit(productCategoryManagerContract, 'ProductCategoryRegistered')
                .withArgs(
                    registeredProductCategory.id,
                    registeredProductCategory.name,
                    registeredProductCategory.quality
                );

            const tx2 = await productCategoryManagerContract.registerProductCategory(
                'testProductCategory2',
                90,
                'secondProductCategory'
            );
            await tx2.wait();
            currentProductCategoryCounter =
                await productCategoryManagerContract.getProductCategoriesCounter();
            expect(currentProductCategoryCounter).to.be.equal(2);
            const registeredProductCategory2 =
                await productCategoryManagerContract.getProductCategory(2);

            expect(tx2)
                .to.emit(productCategoryManagerContract, 'ProductCategoryRegistered')
                .withArgs(
                    registeredProductCategory2.id,
                    registeredProductCategory2.name,
                    registeredProductCategory2.quality
                );
        });
    });
    ...
    describe('Update', () => {
        it('should update a material', async () => {
            await productCategoryManagerContract.registerProductCategory(
                'testProductCategory',
                85,
                'firstProductCategory'
            );
            const tx = await productCategoryManagerContract.updateProductCategory(
                1,
                'testProductCategory2',
                90,
                'secondProductCategory'
            ...
            expect(registeredProductCategory[4]).to.be.equal(true);
            expect(tx)
                .to.emit(productCategoryManagerContract, 'ProductCategoryUpdated')
                .withArgs(registeredProductCategory.id);
        });
    });
    ...
    describe('Roles', () => {
        it('should add and remove admin roles', async () => {
            ...
            expect(
                await productCategoryManagerContract.hasRole(
                    await productCategoryManagerContract.ADMIN_ROLE(),
                    other.address
                )
            ).to.equal(true);
        ...
        it('should fail to add and remove admin roles if the caller is not an admin', async () => {
            await expect(
                productCategoryManagerContract.connect(other).addAdmin(admin.address)
            ).to.be.revertedWith('ProductCategoryManager: Caller is not the admin');
        });
    });
});
