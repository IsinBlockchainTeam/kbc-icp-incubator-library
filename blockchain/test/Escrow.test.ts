...
describe('Escrow.sol', () => {
    ...
    let admin: SignerWithAddress, payee: SignerWithAddress,
        purchaser: SignerWithAddress, delegate: SignerWithAddress,
        commissioner: SignerWithAddress, anotherCommissioner: SignerWithAddress;
    ...
    beforeEach(async () => {
        ...
        escrowContract = await Escrow.deploy(admin.address, payee.address, purchaser.address, agreedAmount, duration, tokenContract.address, commissioner.address, baseFee, percentageFee);
        await escrowContract.deployed();
    });

    describe('Escrow', () => {
        it('should retrieve escrow correctly', async () => {
            expect(await escrowContract.getOwner())
                .to
                .equal(payee.address);
            ...
            expect(payer.depositedAmount)
                .to
                .equal(BigNumber.from(0));
            expect(payer.isPresent).to.be.true;

            expect(await escrowContract.getAgreedAmount())
                .to
                .equal(agreedAmount);
            expect(await escrowContract.getAgreedAmount())
                .to
                .equal(agreedAmount);
            expect(await escrowContract.getDeployedAt())
                .to
                .be
                .closeTo(Math.floor(Date.now() / 1000), 100);
        ...
        it('should fail creating an escrow if payee is the zero address', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            await expect(Escrow.deploy(admin.address, ethers.constants.AddressZero, purchaser.address, agreedAmount, duration, tokenContract.address, commissioner.address, baseFee, percentageFee))
                .to
                .be
                .revertedWith('Escrow: payee is the zero address');
        });

        it('should fail creating an escrow if purchaser is the zero address', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            await expect(Escrow.deploy(admin.address, payee.address, ethers.constants.AddressZero, agreedAmount, duration, tokenContract.address, commissioner.address, baseFee, percentageFee))
                .to
                .be
                .revertedWith('Escrow: purchaser is the zero address');
        });

        it('should fail creating an escrow if token address is the zero address', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            await expect(Escrow.deploy(admin.address, payee.address, purchaser.address, agreedAmount, duration, ethers.constants.AddressZero, commissioner.address, baseFee, percentageFee))
                .to
                .be
                .revertedWith('Escrow: token address is the zero address');
        });

        it('should fail creating an escrow if commissioner is the zero address', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            await expect(Escrow.deploy(admin.address, payee.address, purchaser.address, agreedAmount, duration, tokenContract.address, ethers.constants.AddressZero, baseFee, percentageFee))
                .to
                .be
                .revertedWith('Escrow: commissioner is the zero address');
        });

        it('should fail creating an escrow if percentage fee is greater than 100', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            await expect(Escrow.deploy(admin.address, payee.address, purchaser.address, agreedAmount, duration, tokenContract.address, commissioner.address, baseFee, 101))
                .to
                .be
                .revertedWith('Escrow: percentage fee cannot be greater than 100');
        });
    });

    describe('Close', () => {
        it('should close escrow', async () => {
            const tx = await escrowContract.connect(admin)
                .close();
            await expect(tx)
                .to
        ...
        it('should fail closing escrow if caller is not admin', async () => {
            await expect(escrowContract.connect(purchaser)
                .close())
                .to
                .be
    ...
    describe('Deposit', () => {
        it('should deposit funds', async () => {
            ...
            const tx = await escrowContract.connect(purchaser)
                .deposit(depositAmount);

            await expect(tx)
                .to
                .emit(escrowContract, 'EscrowDeposited')
        ...
        it('should fail deposit if escrow is not in \'Active\' state', async () => {
            ...
            await expect(escrowContract.connect(purchaser)
                ...
                .to
                .be
                .revertedWith('Escrow: can only deposit while active');
        });

        it('should fail deposit if caller is not a payer', async () => {
            await tokenContract.connect(purchaser)
                .approve(escrowContract.address, depositAmount);
            await expect(escrowContract.connect(payee)
                .deposit(depositAmount))
                .to
                .be
                .revertedWith('Escrow: caller is not a payer');
        });
        ...
        it('should fail deposit if amount is not greater than 0', async () => {
            ...
            await expect(escrowContract.connect(purchaser)
                ...
                .to
                .be
                .revertedWith('Escrow: can only deposit positive amount');
        });
    });
    ...
    describe('Delegation', () => {
        it('should allow delegate to deposit funds', async () => {
            await escrowContract.connect(purchaser)
                .addDelegate(delegate.address);
            await tokenContract.connect(delegate)
                .approve(escrowContract.address, depositAmount);
            const tx = await escrowContract.connect(delegate)
                .deposit(depositAmount);
        ...
        it('should remove delegate', async () => {
            ...
            await tokenContract.connect(delegate)
                .approve(escrowContract.address, depositAmount);
            await expect(escrowContract.connect(delegate)
                .deposit(depositAmount))
                .to
                .be
                .revertedWith('Escrow: caller is not a payer');
        ...
        it('should fail if delegate is the zero address', async () => {
            await expect(escrowContract.connect(purchaser)
                .addDelegate(ethers.constants.AddressZero))
                .to
                .be
                .revertedWith('Escrow: delegate is the zero address');
        });
    });
    ...
    describe('Refund enabling', () => {
        it('should enable refund', async () => {
            ...
            await expect(tx)
                .to
                .emit(escrowContract, 'EscrowRefundEnabled');
        });
        ...
        it('should externally enable refund for expired escrow', async () => {
            ...
            await skipToDeadline();
            await escrowContract.connect(purchaser)
                .enableRefundForExpiredEscrow();
            expect(await escrowContract.hasExpired()).to.true;
        });

        it('should fail externally enabling refund for expired escrow if escrow has not expired', async () => {
            expect(await escrowContract.hasExpired()).to.false;
            await expect(escrowContract.enableRefundForExpiredEscrow())
                .to
                .be
                .revertedWith('Escrow: can only externally enable refund when escrow has expired');
        });
    });
    ...
    describe('Refund', () => {
        ...
        it('should refund funds to delegate', async () => {
            ...
            await escrowContract.connect(purchaser)
                .addDelegate(delegate.address);
            await escrowContract.connect(delegate)
                .deposit(depositAmount);
            ...
            expect(await tokenContract.balanceOf(delegate.address))
                .to
                .be
                .greaterThan(delegateInitialBalance);
            expect(await tokenContract.balanceOf(purchaser.address))
                .to
                .be
                .equal(purchaserInitialBalance);
        });
        ...
        it('should refund purchaser and delegate funds if both have deposited', async () => {
            ...
            await escrowContract.connect(purchaser)
                .deposit(depositAmount / 2);
            await escrowContract.connect(delegate)
                .deposit(depositAmount / 4);

            await escrowContract.connect(admin)
                .enableRefund();
            let tx = await escrowContract.connect(purchaser)
                .refund();
            await tx.wait();
            expect(tx)
                .emit(escrowContract, 'EscrowRefunded')
    ...
    describe('Withdraw', () => {
        it('should withdraw funds', async () => {
            ...
            await escrowContract.connect(admin)
                .close();
            const tx = await escrowContract.connect(payee)
                .withdraw();
            await tx.wait();
            expect(tx)
                .emit(escrowContract, 'Withdrawn')
                .withArgs(depositAmount);

            expect(await tokenContract.balanceOf(payee.address))
                .to
                .be
                .greaterThan(initialBalance);
        });
        ...
        it('should fail withdrawing funds if not payee', async () => {
            await expect(escrowContract.connect(purchaser)
                ...
                .to
                .be
                .revertedWith('Escrow: caller is not the payee');
        });
        ...
        it('should fail withdrawing funds if escrow is not in \'Closed\' state', async () => {
            await expect(escrowContract.connect(payee)
                ...
                .to
                .be
                .revertedWith('Escrow: can only withdraw while closed');
        });
    ...
    describe('Fees', () => {
        it('should pay fees on withdrawal', async () => {
            ...
            await escrowContract.connect(purchaser)
                .deposit(depositAmount);
            expect(await tokenContract.balanceOf(commissioner.address))
                .to
                .equal(0);

            await escrowContract.connect(admin)
                .close();
            await escrowContract.connect(payee)
                .withdraw();
            expect(await tokenContract.balanceOf(commissioner.address))
                .to
                .equal(calculateFee(depositAmount));
            expect(await tokenContract.balanceOf(payee.address))
                .to
                .equal(depositAmount - calculateFee(depositAmount));
        });
        ...
        it('should pay fees on refund', async () => {
            ...
            expect(await tokenContract.balanceOf(commissioner.address))
                .to
                .equal(calculateFee(depositAmount) * 2);
        });

        it('should pay fees equal to deposit amount if base fee is equal to deposited amount', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            escrowContract = await Escrow.deploy(admin.address, payee.address, purchaser.address, agreedAmount, duration, tokenContract.address, commissioner.address, depositAmount, percentageFee);
            await escrowContract.deployed();
        ...
        it('should pay fees equal to deposit amount if base fee is greater than deposited amount', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            escrowContract = await Escrow.deploy(admin.address, payee.address, purchaser.address, agreedAmount, duration, tokenContract.address, commissioner.address, depositAmount + 1, percentageFee);
            await escrowContract.deployed();
        ...
        it('should pay fees equal to deposit amount if percentage fee is 100', async () => {
            const Escrow = await ethers.getContractFactory(ContractName.ESCROW);
            escrowContract = await Escrow.deploy(admin.address, payee.address, purchaser.address, agreedAmount, duration, tokenContract.address, commissioner.address, baseFee, 100);
            await escrowContract.deployed();
        ...
        it('should update commission address', async () => {
            ...
            await escrowContract.connect(purchaser)
                .deposit(depositAmount);
            await escrowContract.connect(admin)
                .updateCommissioner(anotherCommissioner.address);
            ...
            expect(await tokenContract.balanceOf(commissioner.address))
                .to
                .equal(0);
            expect(await tokenContract.balanceOf(anotherCommissioner.address))
                .to
                .equal(calculateFee(depositAmount));
        });
        ...
        it('should fail updating commission address if new address is zero address', async () => {
            await expect(escrowContract.connect(admin)
                .updateCommissioner(ethers.constants.AddressZero))
                .to
                .be
                .revertedWith('Escrow: commissioner is the zero address');
        });
