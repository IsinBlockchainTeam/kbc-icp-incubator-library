...
export function calculateRsvForTEcdsa(chainId: number, address: string, digest: string, signature: Uint8Array): { r: string; s: string; v: number } {
    ...
    if (address.toLowerCase() === ethers.recoverAddress(digest, rsv0).toLowerCase()) {
        return rsv0;
    }
    ...
    if (address.toLowerCase() === ethers.recoverAddress(digest, rsv1).toLowerCase()) {
        return rsv1;
    }
