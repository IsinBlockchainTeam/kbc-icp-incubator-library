...
function validateAndExtractParameters(this: any, args: any[]) {
    ...
    if (args[0].delegateAddress === undefined) throw new Error(`First argument must be a RoleProof`);
    if (args[1] === undefined) throw new Error(`Second argument must be the Shipment id`);
...
export function OnlyInvolvedParties(originalMethod: any, _context: any) {
    async function replacementMethod(this: any, ...args: any[]) {
        ...
        if (!interestedParties.includes(callerAddress)) throw new Error('Access denied, user is not an involved party');
...
export function OnlySupplier(originalMethod: any, _context: any) {
    async function replacementMethod(this: any, ...args: any[]) {
        const { shipment, callerAddress } = validateAndExtractParameters.call(this, args);
        if (callerAddress !== shipment.supplier) throw new Error('Access denied, user is not the supplier');
        return originalMethod.call(this, ...args);
    }
    return replacementMethod;
}
...
export function OnlyCommissioner(originalMethod: any, _context: any) {
    async function replacementMethod(this: any, ...args: any[]) {
        const { shipment, callerAddress } = validateAndExtractParameters.call(this, args);
        if (callerAddress !== shipment.commissioner) throw new Error('Access denied, user is not the commissioner');
        return originalMethod.call(this, ...args);
    }
    return replacementMethod;
}
