...
class ShipmentService implements HasInterestedParties {
    ...
    private _shipments = StableBTreeMap<bigint, Shipment>(
        StableMemoryId.SHIPMENTS,
    );
    ...
    async createShipment(
        ...
        await ethSendContractTransaction(
            escrowManagerAddress,
            escrowManagerAbi.abi,
            "registerEscrow",
            [shipment.id, supplier, duration, tokenAddress],
        );
    ...
    getShipmentPhase(id: bigint): Phase {
        ...
        if (
            !this.areDocumentsUploadedAndApproved(
                id,
                this.getPhase1RequiredDocuments(),
            )
        )
            return { PHASE_1: null };
        if (
            shipment.sampleApprovalRequired &&
            !(EvaluationStatusEnum.APPROVED in shipment.sampleEvaluationStatus)
        )
            return { PHASE_1: null };
        ...
        if (
            !this.areDocumentsUploadedAndApproved(
                id,
                this.getPhase2RequiredDocuments(),
            )
        )
            return { PHASE_2: null };
        if (
            !(EvaluationStatusEnum.APPROVED in shipment.detailsEvaluationStatus)
        )
            return { PHASE_2: null };
        ...
        if (
            !this.areDocumentsUploadedAndApproved(
                id,
                this.getPhase3RequiredDocuments(),
            )
        )
            return { PHASE_3: null };
        ...
        if (
            !this.areDocumentsUploadedAndApproved(
                id,
                this.getPhase4RequiredDocuments(),
            )
        )
            return { PHASE_4: null };
        ...
        if (
            !this.areDocumentsUploadedAndApproved(
                id,
                this.getPhase5RequiredDocuments(),
            )
        )
            return { PHASE_5: null };
        if (
            EvaluationStatusEnum.NOT_EVALUATED in
            shipment.qualityEvaluationStatus
        )
            return { PHASE_5: null };
    ...
    private areDocumentsUploadedAndApproved(
        id: bigint,
        ...
        for (const documentType of requiredDocuments) {
            ...
            if (
                document.length === 0 ||
                !(EvaluationStatusEnum.APPROVED in document[0].evaluationStatus)
            )
                return false;
        }
        return true;
    ...
    getDocumentsByType(id: bigint, documentType: DocumentType): DocumentInfo[] {
        const shipment = this.getShipment(id);
        const documentInfos = shipment.documents.find(
            ([type]) => Object.keys(documentType)[0] in type,
        );
    ...
    async determineEscrowAddress(id: bigint): Promise<Shipment> {
        ...
        const escrowManagerAddress: string = getEvmEscrowManagerAddress();
        const escrowAddress = await ethCallContract(
            escrowManagerAddress,
            escrowManagerAbi.abi,
            "getEscrowByShipmentId",
            [shipment.id],
        );
    ...
    async lockFunds(id: bigint): Promise<Shipment> {
        ...
        const escrowAddress = shipment.escrowAddress[0];
        const totalLockedFunds = await ethCallContract(
            escrowAddress,
            escrowAbi.abi,
            "getLockedAmount",
            [],
        ...
        const totalDepositedAmount = await ethCallContract(
            escrowAddress,
            escrowAbi.abi,
    ...
    async addDocument(
        ...
        if (DocumentTypeEnum.GENERIC in documentType) {
            const genericDocuments = shipment.documents.find(
                ([type]) => "GENERIC" in type,
            );
            genericDocuments
                ? genericDocuments[1].push(documentInfo)
                : shipment.documents.push([documentType, [documentInfo]]);
        } else {
            const documentTupleIndex = shipment.documents.findIndex(
                ([type]) => Object.keys(documentType)[0] in type,
            );
            if (documentTupleIndex !== -1) {
                shipment.documents[documentTupleIndex] = [
                    documentType,
                    [documentInfo],
                ];
            } else {
    ...
    async updateDocument(
        ...
        const shipment = this.getShipment(id);
        const documentTuple = shipment.documents.find(([_, docs]) =>
        ...
        const documentIndex = documentTuple[1].findIndex(
    ...
    async evaluateDocument(
        id: bigint,
        documentId: bigint,
        documentEvaluationStatus: EvaluationStatus,
    ): Promise<Shipment> {
        const shipment = this.getShipment(id);
        const documentTuple = shipment.documents.find(([_, docs]) =>
            docs.find((doc) => doc.id === documentId),
        ...
        documentTuple[1][documentIndex].evaluationStatus =
