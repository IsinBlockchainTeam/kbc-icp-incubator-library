...
describe("OrderService", () => {
    ...
    let authenticationServiceInstanceMock =
        AuthenticationService.instance as jest.Mocked<AuthenticationService>;
    let shipmentServiceInstanceMock =
        ShipmentService.instance as jest.Mocked<ShipmentService>;
    ...
    it("retrieves interested parties", () => {
        ...
        expect(() => orderService.getInterestedParties(1n)).toThrow(
            new Error("(ORDER_NOT_FOUND) Order not found."),
        );
    });
    ...
    it("retrieves a order by id", () => {
        ...
        mockedFn.get.mockReturnValue(undefined);
        expect(() => orderService.getOrder(1n)).toThrow(
            new Error("(ORDER_NOT_FOUND) Order not found."),
        );
    });
    ...
    it("creates a order", () => {
        ...
        authenticationServiceInstanceMock.getDelegatorAddress.mockReturnValue(
            delegatorAddress,
        );
        ...
        ).toThrow(new Error("(SAME_ACTORS) Actors are the same."));
    ...
    it("updates a order", () => {
        ...
        ).toThrow(new Error("(ORDER_NOT_FOUND) Order not found."));
        mockedFn.get.mockReturnValue(order);
        expect(() =>
            orderService.updateOrder(
                0n,
        ...
        ).toThrow(new Error("(ORDER_NO_CHANGES) Order with no changes."));
        expect(() =>
            orderService.updateOrder(
                0n,
                order.supplier,
    ...
    it("signs a order", async () => {
        ...
        mockedFn.get.mockReturnValue(order);
        shipmentServiceInstanceMock.createShipment.mockResolvedValue({
            id: 1n,
        } as Shipment);
        ...
        mockedFn.get.mockReturnValue(undefined);
        await expect(() => orderService.signOrder(0n)).rejects.toThrow(
            new Error("(ORDER_NOT_FOUND) Order not found."),
        );
        mockedFn.get.mockReturnValue(order);
        authenticationServiceInstanceMock.getDelegatorAddress.mockReturnValue(
        ...
        await expect(() => orderService.signOrder(0n)).rejects.toThrow(
            new Error("(ORDER_ALREADY_CONFIRMED) Order already confirmed."),
        );
        authenticationServiceInstanceMock.getDelegatorAddress.mockReturnValue(
            "0xother",
        );
