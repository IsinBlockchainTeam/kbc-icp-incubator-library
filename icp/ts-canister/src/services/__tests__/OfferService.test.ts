...
import { ErrorType, Offer, ProductCategory } from '../../models/types';
...
describe('OfferService', () => {
    ...
    it('retrieves a offer by id', () => {
        ...
        expect(() => offerService.getOffer(1n)).toThrow(new Error(`(${ErrorType.OFFER_NOT_FOUND}) Offer not found.`));
    });
    ...
    it('creates a offer', () => {
        ...
        expect(() => offerService.createOffer(0n)).toThrow(new Error(`(${ErrorType.PRODUCT_CATEGORY_NOT_FOUND}) Product category not found.`));
    });
});
