...
import { FundStatusEnum, Phase, PhaseEnum, Shipment, EvaluationStatus, EvaluationStatusEnum, DocumentInfo, DocumentType, DocumentTypeEnum } from '../models/types';
...
class ShipmentService implements HasInterestedParties, HasSupplier, HasCommissioner {
    ...
    async createShipment(supplier: string, commissioner: string, sampleApprovalRequired: boolean, duration: bigint, tokenAddress: string): Promise<Shipment> {
        ...
        await ethSendContractTransaction(downPaymentManagerAddress, downPaymentManagerAbi.abi, 'registerDownPayment', [shipment.id, supplier, duration, tokenAddress]);
    ...
    async determineDownPaymentAddress(id: bigint): Promise<Shipment> {
        ...
        const downPaymentManagerAddress: string = Evm.DOWN_PAYMENT_MANAGER_ADDRESS;
        const downPaymentAddress = await ethCallContract(downPaymentManagerAddress, downPaymentManagerAbi.abi, 'getDownPaymentByShipmentId', [shipment.id]);
