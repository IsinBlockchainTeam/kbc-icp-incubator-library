...
const idlFactory = ({
  ...
  MetadataMap.fill(IDL.Vec(IDL.Tuple(IDL.Text, IDL.Variant({
    'map': MetadataMap,
    'string': IDL.Text,
  ...
  MetadataMapV2.fill(IDL.Vec(IDL.Tuple(IDL.Text, IDL.Variant({
    'Map': MetadataMapV2,
    'String': IDL.Text,
    'Bytes': IDL.Vec(IDL.Nat8)
  }))));
  const PublicKeyAuthn = IDL.Record({
    'pubkey': PublicKey
  });
  ...
  const AuthnMethodAddError = IDL.Variant({
  ...
  const CheckCaptchaArg = IDL.Record({
    'solution': IDL.Text
  });
  const RegistrationFlowNextStep = IDL.Variant({
    'CheckCaptcha': IDL.Record({
      'captcha_png_base64': IDL.Text
    }),
    'Finish': IDL.Null
  });
  ...
  const CheckCaptchaError = IDL.Variant({
    ...
    'UnexpectedCall': IDL.Record({
      'next_step': RegistrationFlowNextStep
    }),
    'WrongSolution': IDL.Record({
      'new_captcha_png_base64': IDL.Text
    })
  });
  ...
  const IdRegFinishResult = IDL.Record({
  ...
  const InternetIdentityStats = IDL.Record({
    ...
    'event_aggregations': IDL.Vec(IDL.Tuple(IDL.Text, IDL.Vec(IDL.Tuple(IDL.Text, IDL.Nat64))))
  });
  const VerifyTentativeDeviceResponse = IDL.Variant({
    'device_registration_mode_off': IDL.Null,
    'verified': IDL.Null,
    'wrong_code': IDL.Record({
      'retries_left': IDL.Nat8
    }),
  ...
  return IDL.Service({
    ...
    'authn_method_add': IDL.Func([IdentityNumber, AuthnMethodData], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': AuthnMethodAddError
    })], []),
    'authn_method_confirm': IDL.Func([IdentityNumber, IDL.Text], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': AuthnMethodConfirmationError
    })], []),
    'authn_method_metadata_replace': IDL.Func([IdentityNumber, PublicKey, MetadataMapV2], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': AuthnMethodMetadataReplaceError
    })], []),
    'authn_method_register': IDL.Func([IdentityNumber, AuthnMethodData], [IDL.Variant({
      'Ok': AuthnMethodConfirmationCode,
      'Err': AuthnMethodRegisterError
    })], []),
    'authn_method_registration_mode_enter': IDL.Func([IdentityNumber], [IDL.Variant({
      'Ok': IDL.Record({
        'expiration': Timestamp
      }),
      'Err': IDL.Null
    })], []),
    'authn_method_registration_mode_exit': IDL.Func([IdentityNumber], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': IDL.Null
    })], []),
    'authn_method_remove': IDL.Func([IdentityNumber, PublicKey], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': IDL.Null
    })], []),
    'authn_method_replace': IDL.Func([IdentityNumber, PublicKey, AuthnMethodData], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': AuthnMethodReplaceError
    })], []),
    'authn_method_security_settings_replace': IDL.Func([IdentityNumber, PublicKey, AuthnMethodSecuritySettings], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': AuthnMethodSecuritySettingsReplaceError
    })], []),
    'check_captcha': IDL.Func([CheckCaptchaArg], [IDL.Variant({
    ...
    'get_id_alias': IDL.Func([GetIdAliasRequest], [IDL.Variant({
    ...
    'identity_authn_info': IDL.Func([IdentityNumber], [IDL.Variant({
      'Ok': IdentityAuthnInfo,
      'Err': IDL.Null
    })], ['query']),
    'identity_info': IDL.Func([IdentityNumber], [IDL.Variant({
      'Ok': IdentityInfo,
      'Err': IdentityInfoError
    })], []),
    'identity_metadata_replace': IDL.Func([IdentityNumber, MetadataMapV2], [IDL.Variant({
      'Ok': IDL.Null,
      'Err': IdentityMetadataReplaceError
    })], []),
    'identity_registration_finish': IDL.Func([IdRegFinishArg], [IDL.Variant({
      'Ok': IdRegFinishResult,
      'Err': IdRegFinishError
    })], []),
    'identity_registration_start': IDL.Func([], [IDL.Variant({
      'Ok': IdRegNextStepResult,
      'Err': IdRegStartError
    })], []),
    'init_salt': IDL.Func([], [], []),
    ...
    'prepare_id_alias': IDL.Func([PrepareIdAliasRequest], [IDL.Variant({
      'Ok': PreparedIdAlias,
      'Err': PrepareIdAliasError
    })], []),
    'register': IDL.Func([DeviceData, ChallengeResult, IDL.Opt(IDL.Principal)], [RegisterResponse], []),
