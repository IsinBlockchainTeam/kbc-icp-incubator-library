...
const idlFactory = ({
  ...
  const Metrics = IDL.Record({
    ...
    'responses': IDL.Vec(IDL.Tuple(IDL.Tuple(IDL.Text, IDL.Text, IDL.Text), IDL.Nat64)),
    'errNoPermission': IDL.Nat64,
    'inconsistentResponses': IDL.Vec(IDL.Tuple(IDL.Tuple(IDL.Text, IDL.Text), IDL.Nat64)),
    ...
    'errHttpOutcall': IDL.Vec(IDL.Tuple(IDL.Tuple(IDL.Text, IDL.Text), IDL.Nat64)),
    'errHostNotAllowed': IDL.Vec(IDL.Tuple(IDL.Text, IDL.Nat64))
  });
  const RpcAuth = IDL.Variant({
    'BearerToken': IDL.Record({
      'url': IDL.Text
    }),
    'UrlParameter': IDL.Record({
      'urlPattern': IDL.Text
    })
  });
  ...
  const RequestResult = IDL.Variant({
    'Ok': IDL.Text,
    'Err': RpcError
  });
  const RequestCostResult = IDL.Variant({
    'Ok': IDL.Nat,
    'Err': RpcError
  });
  return IDL.Service({
    'eth_call': IDL.Func([RpcServices, IDL.Opt(RpcConfig), CallArgs], [MultiCallResult], []),
    'eth_feeHistory': IDL.Func([RpcServices, IDL.Opt(RpcConfig), FeeHistoryArgs], [MultiFeeHistoryResult], []),
    'eth_getBlockByNumber': IDL.Func([RpcServices, IDL.Opt(RpcConfig), BlockTag], [MultiGetBlockByNumberResult], []),
    'eth_getLogs': IDL.Func([RpcServices, IDL.Opt(RpcConfig), GetLogsArgs], [MultiGetLogsResult], []),
    'eth_getTransactionCount': IDL.Func([RpcServices, IDL.Opt(RpcConfig), GetTransactionCountArgs], [MultiGetTransactionCountResult], []),
    'eth_getTransactionReceipt': IDL.Func([RpcServices, IDL.Opt(RpcConfig), IDL.Text], [MultiGetTransactionReceiptResult], []),
    'eth_sendRawTransaction': IDL.Func([RpcServices, IDL.Opt(RpcConfig), IDL.Text], [MultiSendRawTransactionResult], []),
    'getMetrics': IDL.Func([], [Metrics], ['query']),
    'getNodesInSubnet': IDL.Func([], [IDL.Nat32], ['query']),
    'getProviders': IDL.Func([], [IDL.Vec(Provider)], ['query']),
    'getServiceProviderMap': IDL.Func([], [IDL.Vec(IDL.Tuple(RpcService, ProviderId))], ['query']),
    'request': IDL.Func([RpcService, IDL.Text, IDL.Nat64], [RequestResult], []),
    'requestCost': IDL.Func([RpcService, IDL.Text, IDL.Nat64], [RequestCostResult], ['query']),
    'updateApiKeys': IDL.Func([IDL.Vec(IDL.Tuple(ProviderId, IDL.Opt(IDL.Text)))], [], [])
  });
};
