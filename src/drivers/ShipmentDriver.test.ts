...
describe('ShipmentDriver', () => {
    ...
    it('retrieves document info successfully', async () => {
        mockedToNumber.mockReturnValue(1);
        const rawDocumentInfo = [{ toNumber: mockedToNumber }, 0, 0, 'uploader', true];
    ...
    it('throws an error when document id is less than 0', async () => {
        await expect(shipmentDriver.getDocumentInfo(roleProof, -1)).rejects.toThrow(
            'Document ID must be greater than or equal to 0'
        );
    });
    ...
    it('throws an error when deposit amount is less than or equal to 0', async () => {
        await expect(shipmentDriver.depositFunds(roleProof, 0)).rejects.toThrow(
            'Amount must be greater than 0'
        );
    });
    it('adds document successfully', async () => {
        await shipmentDriver.addDocument(
            roleProof,
            DocumentType.SERVICE_GUIDE,
            'http://example.com',
            'hash'
        );
        expect(mockedWriteFunction).toHaveBeenCalledWith(
            roleProof,
            DocumentType.SERVICE_GUIDE,
            'http://example.com',
            'hash'
    ...
    it('throws an error when document id is less than 0', async () => {
        await expect(
            shipmentDriver.evaluateDocument(roleProof, -1, DocumentEvaluationStatus.APPROVED)
        ).rejects.toThrow('Document ID must be greater than or equal to 0');
    });
