...
describe('OrderDriver', () => {
    ...
    it('should create order', async () => {
        ...
        const orderParams = {
            ...
            token: '0xc204d8b5Ee2E2D46387da857ebb0D4fC88438926',
            agreedAmount: 10,
    ...
    it('should create and sign an order', async () => {
        const { companyWallet: company1Wallet, orderManagerDriver, login } = utils1;
        await login();
        const { companyWallet: company2Wallet } = utils2;
        const date = new Date();
        ...
        console.log(order);
        expect(order).toBeDefined();
        const { orderManagerDriver: orderManagerDriver2, login: login2 } = utils2;
        await login2();
        const signedOrder = await orderManagerDriver2.signOrder(order.id);
