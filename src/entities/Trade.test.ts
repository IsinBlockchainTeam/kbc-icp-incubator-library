...
describe('Line', () => {
    ...
    it('should correctly initialize a Line', () => {
        expect(line.id).toEqual(0);
        expect(line.material).toEqual(new Material(1, new ProductCategory(1, 'test', 1, 'test')));
        expect(line.productCategory).toEqual(new ProductCategory(1, 'test', 1, 'test'));
        expect(line.quantity).toEqual(10);
        expect(line.unit).toEqual('KGM');
    });

    it('should correctly set the id', () => {
        line.id = 42;
        expect(line.id).toEqual(42);
    });

    it('should correctly set the material', () => {
        line.material = new Material(2, new ProductCategory(2, 'test2', 2, 'test2'));
        expect(line.material).toEqual(new Material(2, new ProductCategory(2, 'test2', 2, 'test2')));
    });

    it('should correctly set the product category', () => {
        line.productCategory = new ProductCategory(2, 'test2', 2, 'test2');
        expect(line.productCategory).toEqual(new ProductCategory(2, 'test2', 2, 'test2'));
    });

    it('should correctly set the quantity', () => {
        line.quantity = 20;
        expect(line.quantity).toEqual(20);
    });

    it('should correctly set the unit', () => {
        line.unit = 'BG';
        expect(line.unit).toEqual('BG');
    });
});
...
describe('LineRequest', () => {
    ...
    it('should correctly initialize a LineRequest', () => {
        expect(line.productCategoryId).toEqual(1);
        expect(line.quantity).toEqual(15);
        expect(line.unit).toEqual('KGM');
    });

    it('should correctly set the product category', () => {
        line.productCategoryId = 2;
        expect(line.productCategoryId).toEqual(2);
    });
...
describe('Trade', () => {
    ...
    it('should correctly set the lines', () => {
        ...
        trade.lines = [
            EntityBuilder.buildTradeLine(newLine, productCategoryStruct, materialStruct)
        ];
        expect(trade.lines).toEqual([
            EntityBuilder.buildTradeLine(newLine, productCategoryStruct, materialStruct)
        ]);
