...
export class GraphService {
    ...
    constructor(
        ...
        transformationService: AssetOperationService
    ) {
        this._signer = signer;
    ...
    private _handleConsolidations(
        ...
        const transformation: AssetOperation = assetOperations.filter(
            (ao: AssetOperation) => ao.type === AssetOperationType.TRANSFORMATION
        )[0];
        if (
            assetOperations.some(
                (ao: AssetOperation) => ao.type === AssetOperationType.CONSOLIDATION
            )
        ) {
            const consolidations: AssetOperation[] = assetOperations.filter(
                (ao) => ao.type === AssetOperationType.CONSOLIDATION
            );
            const trades: Trade[] = this._trades.filter((t: Trade) =>
                t.lines.some((l: Line) => l.material?.id === materialId)
            );
            ...
            for (let i: number = 0; i < consolidations.length; i++) {
                ...
                partialGraphData.edges.push({
                    ...
                    from:
                        i === consolidations.length - 1
                            ? transformation.name
                            : consolidations[i + 1].name,
    ...
    private _computeGraph(
        ...
        for (const assetOperationInputMaterial of transformation.inputMaterials) {
            const filteredTradesMap: Map<Trade, number[]> = this._trades
                .filter((t: Trade) =>
                    t.lines.some((l: Line) => l.material?.id === assetOperationInputMaterial.id)
                )
                .reduce((acc: Map<Trade, number[]>, trade: Trade) => {
                    acc.set(
                        trade,
                        trade.lines
                            .filter((l: Line) => l.material?.id === assetOperationInputMaterial.id)
            ...
            for (const [trade, materialInputIds] of filteredTradesMap) {
                for (const involvedMaterialInputId of materialInputIds) {
                    ...
                    const inputAssetOperation: AssetOperation | undefined = list
                        ? list[0]
                        : undefined;

                    if (!inputAssetOperation)
                        throw new Error(
                            'Found a trade whose material id is not associated with any asset operation'
                        );

                    if (
                        inputAssetOperation.type === AssetOperationType.CONSOLIDATION &&
                        partialGraphData.nodes.some(
                            (ao: AssetOperation) => ao.id === inputAssetOperation.id
                        )
                    )
                        return partialGraphData;
                    ...
                    if (
                        partialGraphData.edges.some(
                            (e) =>
                                e.trade === newEdge.trade &&
                                e.from === newEdge.from &&
                                e.to === newEdge.to
                        )
                    )
