...
export class CertificationService {
    ...
    async getScopeCertificatesByProcessType(subject: string, processType: string): Promise<ScopeCertificate[]> {
        const certificates = await this._certificationManagerDriver.getScopeCertificates(subject);
        return certificates.filter((certificate) => certificate.processTypes.includes(processType));
    }
    ...
    async getMaterialCertificatesByMaterialId(subject: string, materialId: number): Promise<MaterialCertificate[]> {
        const certificates = await this._certificationManagerDriver.getMaterialCertificates(subject);
        return certificates.filter((certificate) => certificate.material.id === materialId);
    }
    ...
    async _addDocument(relativePath: string, document: Document): Promise<string> {
        const baseExternalUrl = `${FileHelpers.ensureTrailingSlash(document.storageConfig.urlStructure.prefix) +
